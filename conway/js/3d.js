...
Mesh.meshes = {
     ...
     * @type {function}
     * @param {number} s - Size of triangle
     * @returns {Mesh}
     */
     ...
     * @type {function}
     * @param {Vertex} v1 - first point
     * @param {Vertex} v2 - second point
     * @returns {Mesh}
     */
    line: function(v1, v2) {
        return new Mesh({
    ...
    /**
     * Generate a cuboid from two corner vertices.
     * @type {function}
     * @param {Vertex} vertex1 - One corner vertex of the cuboid.
     * @param {Vertex} vertex2 - The diagonally opposite corner vertex of the
     * cuboid.
     * @returns {Mesh} The generated cuboid mesh.
     */
    cuboid: function(vertex1, vertex2) {
        // Calculate the differences in each dimension
        let dx = vertex2.x - vertex1.x;
        let dy = vertex2.y - vertex1.y;
        let dz = vertex2.z - vertex1.z;

        // Generate the vertices of the cuboid
        let vertices = [
            new Vertex(vertex1.x, vertex1.y, vertex1.z),
            new Vertex(vertex1.x + dx, vertex1.y, vertex1.z),
            new Vertex(vertex1.x, vertex1.y + dy, vertex1.z),
            new Vertex(vertex1.x, vertex1.y, vertex1.z + dz),
            new Vertex(vertex1.x + dx, vertex1.y + dy, vertex1.z),
            new Vertex(vertex1.x + dx, vertex1.y, vertex1.z + dz),
            new Vertex(vertex1.x, vertex1.y + dy, vertex1.z + dz),
            new Vertex(vertex1.x + dx, vertex1.y + dy, vertex1.z + dz),
        ];

        // Define the faces based on the vertices
        let faces = [
            [0, 1, 5, 3],
            [0, 3, 6, 2],
            [0, 2, 4, 1],
    ...
    /**
     * Generate a sphere or a segment of a sphere.
     * @type {function}
     * @param {number} radius - The radius of the sphere.
     * @param {number} slices - The total number of slices in the complete sphere.
     * @param {number} stacks - The total number of stacks in the complete sphere.
     * @param {number} [num_slices] - The number of slices to actually use.
     * @param {number} [num_stacks] - The number of stacks to actually use.
     * @returns {Mesh}
     */
    sphere: function(radius, slices, stacks, num_slices, num_stacks) {
        if (num_slices === undefined) {
            num_slices = slices;
        } if (num_stacks === undefined) {
            num_stacks = stacks;
        }

        let vertices = [];

        // Adjust loop for stacks
        for (let stack = 0; stack <= num_stacks; stack++) {
            let phi = Math.PI / stacks * stack; // angle from the top
            let ringRadius = Math.sin(phi) * radius;
            let y = Math.cos(phi) * radius;

            // Adjust loop for slices
            for (let slice = 0; slice <= num_slices; slice++) {
                let theta = 2 * Math.PI / slices * slice; // angle around the sphere
        ...
        for (let stack = 0; stack < num_stacks; stack++) {
            for (let slice = 0; slice < num_slices; slice++) {
                let first = (stack * (num_slices + 1)) + slice;
                let second = first + num_slices + 1;

                faces.push([first, second, second + 1, first + 1]);
            }
        }

        let rotation = new Rotation(Math.PI/2, 0, 0);
        return rotation.apply_rotation_to_mesh(new Mesh({
            vertices: vertices,
            edges: [],
            faces: faces,
        }));
    ...
    /**
     * Generate cylinder with rectangular side segments and whole circular top and bottom faces.
     * @type {function}
     * @param {number} height - The height of the cylinder.
     * @param {number} segments - The number of segments around the circumference.
     * @param {number} bottomRadius - The radius of the bottom circle.
     * @param {number} topRadius - The radius of the top circle.
     * @returns {Mesh}
     */
    cylinder: function (height, segments, bottomRadius, topRadius) {
        let vertices = [];
        let faces = [];
        ...
        for (let i = 0; i <= 1; i++) {
            let y = i * height;
            let radius = i === 0 ? bottomRadius : topRadius;
        ...
        for (let i = 0; i < segments; i++) {
            let currentBottom = i;
            let nextBottom = (i + 1) % segments;
            let currentTop = segments + i;
            let nextTop = segments + ((i + 1) % segments);

            faces.push([currentBottom, nextBottom, nextTop, currentTop]);
        }
        ...
        return rotation.apply_rotation_to_mesh(new Mesh({
            ...
            edges: [],
            faces: faces,
        })).translate(new Vector(0, 0, - height/2));
    }
}
...
/**
 * Get bounding box of mesh. Returns two vectors containing the corners of the
 * bounding box.
 * @return {Vector[]}
 */
Mesh.prototype.bounding_box = function() {

    let dims = ['x','y','z'];
    let low_c = new Vector(null, null, null);
    let high_c = new Vector(null, null, null);

    for (let i = 0; i < dims.length; i++) {
        let c_dim = dims[i]
        for (let x = 0; x < this.vertices.length; x++) {
            let c_vert_dim = this.vertices[x][c_dim];
            if (low_c[c_dim] === null || c_vert_dim < low_c[c_dim])
                low_c[c_dim] = c_vert_dim
            if (high_c[c_dim] === null || c_vert_dim > high_c[c_dim])
                high_c[c_dim] = c_vert_dim
        }
    }
...
/**
 * Get x, y and z lengths of mesh
 * @return {Vector}
 */
Mesh.prototype.size = function() {

    let dims = ['x','y','z'];
    let mesh_size = new Vector(null, null, null);
    ...
    for (let i = 0; i < dims.length; i++) {
        ...
        let lowest = null;
        for (let x = 0; x < this.vertices.length; x++) {
            let current_vert_dim = this.vertices[x][current_dim];
            if (lowest === null || current_vert_dim < lowest)
                lowest = current_vert_dim
            if (highest === null || current_vert_dim > highest)
                highest = current_vert_dim
        }
        mesh_size[current_dim] = highest - lowest
    }
