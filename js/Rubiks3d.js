...
Rubiks3d.Cube = function(
               scene,
               cubeSize = 2,
               cubeString = 'UUUUUUUUUFFFFFFFFFDDDDDDDDDLLLLLLLLLRRRRRRRRRBBBBBBBBB',
               startingPosition = [0, 0, 0],
               spacingPercentage = 0,
               lineWidth = 1,
               colorMappings = 'default') {
    /**
     * Cube
    ...
    // create mappings for x, y and z values matching with UFDLRB values in cubestring
    this.cubeStringMappings = {
        FB: {y: { 0: [6, 7, 8], 1: [3, 4, 5], 2: [0, 1, 2] }, x: { 0: [0, 3, 6], 1: [1, 4, 7], 2: [2, 5, 8] }},
        UD: {x: { 0: [0, 3, 6], 1: [1, 4, 7], 2: [2, 5, 8] }, z: { 0: [0, 1, 2], 1: [3, 4, 5], 2: [6, 7, 8] }},
        RL: {y: { 0: [6, 7, 8], 1: [3, 4, 5], 2: [0, 1, 2] }, z: { 0: [0, 3, 6], 1: [1, 4, 7], 2: [2, 5, 8] }}
    }
    ...
    this._createMaterialArray = function(materials) {
        ...
        for (var i = 0; i < 6; i++) {
            materialsArray.push( new THREE.MeshBasicMaterial({ 
                color: materials[i],
                polygonOffset: true,    // used to offset the wireframe a bit
            ...
            for (var y = 0; y < 3; y++) {
                for (var z = 0; z < 3; z++) {
                    ...
                    var cubeGeometry = new THREE.BoxGeometry(
                        this.cubeletSize, this.cubeletSize, this.cubeletSize
                    ) 
                    
                    // generate materials for each face, materials must be in order: [x+, x-, y+, y-, z+, z-]
                    var materials = new Array(6);
                    var index;
                    
                    if (z == 0) {   // must be back side (B)
                    ...
                    } if (z == 2) { // must be front side (F)
                        index = this._intersection(this.cubeStringMappings.FB.y[y], this.cubeStringMappings.FB.x[x])[0]
                        materials[4] = colorMappings[cubeStringObject.F[index]];
                    } if (y == 0) { // must be down side (D)
                    ...
                    } if (y == 2) { // must be up side (U)
                        index = this._intersection(this.cubeStringMappings.UD.z[z], this.cubeStringMappings.UD.x[x])[0]
                        materials[2] = colorMappings[cubeStringObject.U[index]];
                    } if (x == 0) { // must be left side (L)
                    ...
                    // set cube position and add to scene
                    cube.position.set(
                        x * this.cubeletSize + x * this.cubeSpacing + this.startingPosition[0],
                        y * this.cubeletSize + y * this.cubeSpacing + this.startingPosition[1],
                        z * this.cubeletSize + z * this.cubeSpacing + this.startingPosition[2]
                    );
                    cube.visible = this.visible;
                    scene.add(cube);
                    
                    // construct cubelet and push to returning array
